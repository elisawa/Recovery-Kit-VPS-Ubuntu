...
if docker info >/dev/null 2>&1; then
  IMAGE_LIST="$BACKUP_DIR/manifest/docker-images.txt"
  if [[ -f "$IMAGE_LIST" ]]; then
    while read -r repository tag digest rest; do
    ...
    echo "  WARNING: docker-images.txt not found"
  fi
else
...
echo "Backup structure: OK"
  ...
  echo "SHA-256: OK"
  echo
echo "No files, containers, databases, services,"
  echo "or firewall rules were changed."
  echo
  exit 0
fi
...
echo
echo "[3/9] Restoring Docker data..."
...
echo
echo "[4/9] Restoring Docker volumes..."
...
if ! docker exec postgres psql -U admin -d postgres -tAc "SELECT 1 FROM pg_roles WHERE rolname='n8n'" | grep -q 1; then error "PostgreSQL role n8n was not restored."; fi
if ! docker exec postgres psql -U admin -d postgres -tAc "SELECT 1 FROM pg_database WHERE datname='n8n_konten'" | grep -q 1; then docker exec postgres psql -U admin -d postgres -c 'CREATE DATABASE n8n_konten OWNER n8n;'; fi
...
echo
echo "[6/9] Restoring Render and TTS..."
...
if id zkonten >/dev/null 2>&1; then chown -R zkonten:zkonten /opt/render-service /opt/tts-service; fi

echo
echo "[7/9] Restoring system configuration..."
...
echo "SSH configuration intentionally NOT restored."
...
echo "============================================================"
echo
echo "Verify n8n login/workflows, NPM Proxy Hosts/SSL, Render/TTS, and SSH before closing the session."
echo