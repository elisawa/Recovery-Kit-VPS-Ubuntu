...
if [[ -e "$BACKUP_DIR" ]]; then
  n=2
  while [[ -e "${BACKUP_DIR}-${n}" ]]; do ((n++)); done
  BACKUP_DIR="${BACKUP_DIR}-${n}"
fi

if [[ $EUID -ne 0 ]]; then echo "ERROR: jalankan dengan sudo."; exit 1; fi
...
{
 echo "BACKUP_NAME=${BACKUP_NAME}"; echo "BACKUP_DATE=$(date -Is)"; echo "HOSTNAME=$(hostname)"
...
rm -rf "$BACKUP_DIR/docker/compose/stacks/n8n/data" "$BACKUP_DIR/docker/compose/stacks/postgres/data" "$BACKUP_DIR/docker/compose/stacks/cloudbeaver/data" "$BACKUP_DIR/docker/compose/stacks/uptime-kuma/data" "$BACKUP_DIR/docker/compose/stacks/npm/data" "$BACKUP_DIR/docker/compose/stacks/npm/letsencrypt" "$BACKUP_DIR/docker/compose/stacks/dockge/data" "$BACKUP_DIR/docker/compose/npm/data" "$BACKUP_DIR/docker/compose/npm/letsencrypt" "$BACKUP_DIR/docker/compose/dockge/data"
...
if docker volume inspect portainer_portainer_data >/dev/null 2>&1; then docker run --rm -v portainer_portainer_data:/source:ro -v "$BACKUP_DIR/docker/volumes":/backup alpine sh -c 'tar czf /backup/portainer_portainer_data.tar.gz -C /source .'; fi
if docker volume inspect n8n-sandbox_sandbox-tls >/dev/null 2>&1; then docker run --rm -v n8n-sandbox_sandbox-tls:/source:ro -v "$BACKUP_DIR/docker/volumes":/backup alpine sh -c 'tar czf /backup/n8n-sandbox_sandbox-tls.tar.gz -C /source .'; fi
...
[[ -x /opt/render-service/venv/bin/pip ]] && /opt/render-service/venv/bin/pip freeze > "$BACKUP_DIR/services/render-service/requirements.freeze.txt" || true
...
[[ -x /opt/tts-service/venv/bin/pip ]] && /opt/tts-service/venv/bin/pip freeze > "$BACKUP_DIR/services/tts-service/requirements.freeze.txt" || true
...
[[ -d /etc/fail2ban ]] && tar -czf "$BACKUP_DIR/system/fail2ban.tar.gz" -C /etc fail2ban || true
...
{
 echo "===== docker images ====="; docker images --digests; echo
 echo "===== container image IDs ====="
 for c in n8n postgres npm cloudbeaver portainer dockge uptime-kuma; do docker inspect "$c" --format '{{.Name}} | image={{.Config.Image}} | image_id={{.Image}}' 2>/dev/null || true; done
 echo; echo "===== sandbox containers ====="
 docker ps -a --format '{{.Names}}' | while read -r c; do case "$c" in sandbox*) docker inspect "$c" --format '{{.Name}} | image={{.Config.Image}} | image_id={{.Image}}';; esac; done
} > "$BACKUP_DIR/manifest/docker-images.txt"
mkdir -p "$BACKUP_DIR/manifest/compose-rendered"
for compose in /opt/stacks/n8n-sandbox/compose.yaml /opt/stacks/n8n/compose.yaml /opt/stacks/postgres/compose.yaml /opt/stacks/uptime-kuma/compose.yaml /opt/stacks/cloudbeaver/compose.yaml /opt/dockge/compose.yaml /opt/content-factory/docker/portainer/docker-compose.yml /opt/npm/compose.yaml; do
 if [[ -f "$compose" ]]; then
  name="$(basename "$(dirname "$compose")")"
  docker compose -f "$compose" config 2>/dev/null | sed -E -e 's/(PASSWORD|API_KEY|TOKEN|SECRET|KEY):[[:space:]]*[^[:space:]]+/\1: "***REDACTED***"/Ig' -e 's/(PASSWORD|API_KEY|TOKEN|SECRET|KEY)=([^[:space:]]+)/\1=***REDACTED***/Ig' > "$BACKUP_DIR/manifest/compose-rendered/${name}.yaml" || true
 fi
done

echo "[10/10] Membuat checksum..."
(cd "$BACKUP_DIR"; find . -type f ! -name 'checksums.sha256' -print0 | sort -z | xargs -0 sha256sum) > "$BACKUP_DIR/manifest/checksums.sha256"
{
 echo "VPS RECOVERY BACKUP"; echo "Backup Name: ${BACKUP_NAME}"; echo "Created: $(date -Is)"; echo "Host: $(hostname)"; echo "Backup: $BACKUP_DIR"; echo; echo "SIZE:"; du -sh "$BACKUP_DIR"; echo "FILES:"; find "$BACKUP_DIR" -type f | wc -l
} > "$BACKUP_DIR/manifest/README.txt"

echo; echo "============================================================"; echo " BACKUP SELESAI"; echo "============================================================"; echo "Lokasi: $BACKUP_DIR"; echo "Ukuran: $(du -sh "$BACKUP_DIR" | cut -f1)"; echo "File: $(find "$BACKUP_DIR" -type f | wc -l)"; echo "Checksum: $BACKUP_DIR/manifest/checksums.sha256"; echo; echo "PENTING: Salin backup ke storage DI LUAR VPS."

echo; echo "[CLEANUP] Menyimpan hanya 2 backup terbaru..."
mapfile -t OLD_BACKUPS < <(find "$BACKUP_ROOT" -mindepth 1 -maxdepth 1 -type d -name 'vps-backup-*' -printf '%T@ %p\n' | sort -nr | tail -n +3 | cut -d' ' -f2-)
for old in "${OLD_BACKUPS[@]}"; do [[ -n "$old" ]] || continue; echo "  Removing: $old"; rm -rf -- "$old"; done
